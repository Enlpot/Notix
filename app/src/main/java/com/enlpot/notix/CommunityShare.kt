...
/**
 * Links into the community rules repository (github.com/anujja/LessNotice-rules).
 *
...
object CommunityShare {
    const val REPO_URL = "https://github.com/anujja/LessNotice-rules"
     ...
     * different mode or action still counts as user-authored.
     */
    data class NudgeSignature(
        val sourcePackages: List<SourceApp>,
        val mode: MatchMode,
        val includeKeywords: List<String>,
        val excludeKeywords: List<String>,
        val extraCondition: ExtraCondition,
        // 阶段2A：数据模型切换为 actions（List 顺序参与签名）
    ...
    fun signatureOf(rule: BlockerRule) = NudgeSignature(
        rule.sourcePackages,
        rule.condition.mode,
        rule.condition.includeKeywords,
        rule.condition.excludeKeywords,
        rule.extraCondition,
        rule.actions,
    )
    ...
    fun sharePayload(rule: BlockerRule, locale: String? = null): SharePayload {
        ...
        val firstSource = rule.sourcePackages.firstOrNull()
        val appLabel = firstSource?.appName?.takeIf { it.isNotBlank() }
            ?: firstSource?.packageName
            ?: ""
        val pkgLabel = if (firstSource == null) "" else firstSource.packageName
        val body = buildString {
            appendLine("**App:** $appLabel (`$pkgLabel`)")
            appendLine()
