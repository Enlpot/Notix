...
object RuleMutations {
    ...
    fun applyResetHitCounts(current: List<BlockerRule>, ruleIds: List<String>): List<BlockerRule> =
        if (ruleIds.isEmpty()) applyResetHitCounts(current)
        else current.map { if (it.id in ruleIds) it.copy(hitCount = 0) else it }
}
